...
VvMailsWindow = VvTabWindow.extend({
    _tabBodyLayout: null,
    _tabList: ["Thư đến", "Đã gửi", "Soạn thư", "Thư góp ý"],
    _playerName:"",
    ctor: function (playerName) {
        this._super("Thư tín", cc.size(700, 530), this._tabList.length, this._tabList);
    ...
    drawUIWithTab: function(tabIndex, title, content) {
        ...
        this._tabBodyLayout.y = this.getBodySize().height/2 - 40;
    ...
    initMailEditorTab: function (title, content, receiver) {
        ...
        lblTo.setTextColor(BkColor.VV_REGISTER_PHONE_WD_TEXT_COLOR);
        ...
        drawLineBottom.drawSegment(cc.p(38, txtContent.y - txtContent.height / 2 - 8), cc.p(tabBodyWidth - 38, txtContent.y - txtContent.height / 2 - 8), 0.5, BkColor.MAIL_EDITOR_BORDER_COLOR);
            ...
            function (sender, type) {
                if (type == ccui.Widget.TOUCH_ENDED) {
                    ...
                    if(receivers.trim().length === 0){
                        showToastMessage("Bạn chưa nhập tên người nhận", screenSize.width/2 + 20, screenSize.height/2 + 130, 2, 220);
                        txtTo.setFocus();
                        return;
                    }
                    if (mailTitle.trim().length === 0){
                        showToastMessage("Bạn chưa nhập tiêu đề", screenSize.width/2 + 20, screenSize.height/2 + 90, 2);
                        txtSubject.setFocus();
                        return;
                    }
                    if (mailContent.trim().length === 0){
                        showToastMessage("Bạn chưa nhập nội dung", screenSize.width/2 + 20, screenSize.height/2 + 40, 2);
                        txtContent.setFocus();
                        return;
                    }
        ...
        if(receiver && receiver.toLowerCase() == "admin"){
    ...
    drawMailsList: function (lstMails, tabIndex) {
        if (lstMails.length > 0) {
            ...
            this._tabBodyLayout.x = this._tabBodyLayout.x + 46;
        } else {
