 ...
 */
BkDealingCard1 = cc.Class.extend({
    // const
    DELTA_X:							    15,
    MAX_DIVIDE_CARDS: 					    20,
    ...
    ctor:function(gt,ig,bcC,bcName,bcP,bcPos,rc,PlAvatar){
        ...
        this.ingame.addChild(this.bocCaiCard,10000);
    ...
    dealRightCards:function(){
        ...
        var listAction = [];
        var i;
        for (i=this.MAX_DIVIDE_CARDS;i>0;i--){
            listAction.push(this.getActionDealCardRight(i))   ;
            //listAction.push(cc.delayTime(this.BASE_DUARATION_ANIMATION));
        }
        var sq = cc.sequence(listAction);
        this.MoRight.runAction(sq);
    },
    getActionDealCardRight:function(i){
        var tl;
        var pos = ((this.MAX_DIVIDE_CARDS - i) % 5) + 1;
        var bP;
        var eP;
        ...
        } else if (i == this.START_RIGHT_SHARE){
            completeFunc = function(){
                self.DealingLeftNew();
                self.onCompleteMoveRight();
            };
        ...
        tl = this.MoRight.createActionMoveFromTo(bP.x,eP.x,bP.y,eP.y,this.BASE_DUARATION_ANIMATION,completeFunc);
        return tl;
    },
    ...
    finisDealRight:function(){
        this.MoRight.moveFromTo(this.MoRight.x,c.WINDOW_WIDTH/2 + this.DELTA_X,this.MoRight.y,c.WINDOW_HEIGHT/2,this.BASE_DUARATION_ANIMATION);
    },
    DealingLeftNew:function(){
        if (!this.MoLeft.visible){
            this.MoLeft.visible = true;
        }
        var i;
        var listActionLeft = [];
        for (i=this.MAX_DIVIDE_CARDS;i>0;i--){
            listActionLeft.push(this.getActionDealCardLeft(i));
            //listActionLeft.push(cc.delayTime(this.BASE_DUARATION_ANIMATION));
        }
        var sq = cc.sequence(listActionLeft);
        this.MoLeft.runAction(sq);
    },
    getActionDealCardLeft:function(i){
        var pos = ((this.MAX_DIVIDE_CARDS - i) % 5) + 1;
        var bP;
        var eP;
        ...
        var tl = this.MoLeft.createActionMoveFromTo(bP.x,eP.x,bP.y,eP.y,this.BASE_DUARATION_ANIMATION,completeFunc);
        return tl;
    },
    ...
    finisDealLeft:function(){
        ...
        this.MoLeft.moveFromTo(this.MoLeft.x,this.CENTER_POINT.x + this.DELTA_X,this.MoLeft.y,this.CENTER_POINT.y,this.BASE_DUARATION_ANIMATION,f);
    },
    joinCards:function(){
    ...
    doBocCai:function(){
        ...
        ActionChooseBaiCai = function(cc){
            ...
            //self.ingame.addChild(self.bocCaiCard,zo+1);
            self.bocCaiCard.SetSize(108);
            self.bocCaiCard.setLocalZOrder(zo*10);
    ...
    processMoveCardToPlayer:function(){
        ...
        //iCCard.moveFromTo(iCCard.x,iPoint.x,iCCard.y,iPoint.y,timeMoveToPosPlayer);
        //this.bocCaiCard.moveFromTo(this.bocCaiCard.x,iPoint.x,this.bocCaiCard.y,iPoint.y,timeMoveToPosPlayer,f);
        iCCard.delayRunaction(iCCard.createActionMoveFromTo(iCCard.x,iPoint.x,iCCard.y,iPoint.y
                ,timeMoveToPosPlayer),this.TIME_DELAY_MOVE);
        this.bocCaiCard.delayRunaction(this.bocCaiCard.createActionMoveFromTo(this.bocCaiCard.x,iPoint.x,this.bocCaiCard.y,iPoint.y
                ,timeMoveToPosPlayer,f),this.TIME_DELAY_MOVE);
    },
    isMeBocCai:function(){
